 ...
 */
public class GenerateEmailReports implements IReporter {
  private static final Logger L = Logger.getLogger(GenerateEmailReports.class);
  ...
  private void generateForResult(ITestResult ans, ITestNGMethod method, int resultSetSize) {
    ...
    if (hasReporterOutput||hasThrowable) {
      ...
      if (hasReporterOutput) {
        if(hasThrowable) {
          m_out.println("<h3>Screen Shot </h3>");
        }
        for (String line : msgs) {
           m_out.println(line.replace("&amp;", "") + "<br/>");
        }
      }
      if(hasThrowable) {
  ...
  protected void startHtml(PrintWriter out) {
    ...
    out.println("<head>");
    out.println("<title>BTREE: Smoke Testcase Results </title>");
